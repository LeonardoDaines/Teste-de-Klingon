...
//Pega cada palavra do texto
foreach (var texto in textB.Split(" "))
{
    var ultimaLetra = texto[texto.Length - 1];
    var primeiraLetra = texto[0];

    //Texto B, quantas preposições existem? 
    //Resposata primeira pergunta
    if (texto.Length == 3 && !foo.Contains(ultimaLetra) && !texto.Contains("d"))
    {
        preposicoes++;
    }


    //Texto B, quantos são os verbos?
    //Resposata segunda pergunta
    if (texto.Length >= 8 && foo.Contains(ultimaLetra))
    {
        verbos++;

        if(texto.Length >= 8 && foo.Contains(ultimaLetra) && !foo.Contains(primeiraLetra)){
            verPrimPessoa++;
...
//Resposata terceira pergunta
foreach (var palavra in textB.Split(" "))
    {
        //Evita a repetição das palavras no texto
...
var textOrdemCorreta = new StringBuilder("");
...
foreach (var palavra in textB.Split(" "))
{
    ...
    {  
        letras.Append( alfabeto[Array.IndexOf(alfabetoKlingon, letra)]);
    }
        vocabulario.Add(palavra, letras.ToString());
}

        var vocabularioOrdenado = vocabulario.OrderBy(x => x.Value);
            foreach (var item in vocabularioOrdenado)
            {
                textOrdemCorreta.Append(item.Key + " ");
            }
            textOrdemCorreta.Remove(textOrdemCorreta.Length - 1, 1);
           


// Texto B, quantos números bonitos distintos existem?
//Resposata quarta pergunta
...
Console.WriteLine($"A quantidade de preposições é {preposicoes}");
Console.WriteLine($"A quantidade de verbos é {verbos}, dos quais {verPrimPessoa} estão em primeira pessoa.");
